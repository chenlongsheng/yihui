...
public class TAlarmPolicyService extends CrudService<TAlarmPolicyDao, TAlarmPolicy> {
    ...
    public List<MapEntity> getChNames(String devType) {

        List<MapEntity> getChName = dao.getChNames(devType);
        for (MapEntity mapEntity : getChName) {

            List<MapEntity> getChNameDetails = dao.getChNameDetails(mapEntity.get("chType").toString(), mapEntity.get("typeId").toString());

            mapEntity.put("getChannelLevels", getChNameDetails);
